...
1. 使用KubeEdge在边缘节点上编排应用，有什么独特的专有的优点？
1. 如果边缘设备挂了，APP如何会自动跳转到远端服务？
1. 实际应用中推荐使用边缘云还是云边缘？
1. KubeEdge节点和普通节点都在 kubectl get node 中获取到，这两者有什么区别？
1. KubeEdge的安全层是怎么做的？
1. kubeEdge的业务流和serviceBus、edgeHub有交互吗？
1. k3s把多个二进制打包成一个二进制，除了安装方便外，有别的好处吗？
1. K3S的轻量化性能如何保证？
1. 边缘计算与协同边缘计算有什么区别？
1. k0s的主要思想是什么
1. 边缘云和云边缘哪个方案在生产系统多一些
1. k3s和k8s性能相比差距有多少
1. kata解决方案有哪些优点
1. K3S在轻量化的情况下，如何确保安全性？
1. K3S边缘云是否可以通过flannel与K8S集群实现容器网络互通？
1. 边缘节点离线后，依赖数据多次变更，边缘应用和云端数据的最终一致是怎么保证的？
1. 边缘节点的Docker容器镜像是从整个云-边k8s系统统一的镜像仓库拉取的吗？
1. k8s集群生命周期管理是否适用于k3s？
1. k3s相比于k8s的轻量级体现在哪里，能将镜像更轻量化吗？
1. k3s能解决镜像在国外拉不到的问题吗
1. k3s除了安全还有什么优势吗
1. k3s中cloud core可以对应什么规模的edgecore
1. K3s是如何保证数据安全的?与主控节点通信可以中断多长时间
1. 云边缘和边缘云两者从用户角度来讲，是否有差别，哪个用户感觉更好一点
1. 对于开源license，比如GPL、Apache等是否有对国内限制的可能？如果有，目前国产捐献内容是否有不可用风险
1. StarlingX目前是否还有应用场景
1. kubeedge离线自治大概能够保证多久离线时间业务是正常的
1. k3s相比k8s主要在哪些组件有所精简？
1. k3s与kubeedge应用前景怎么样？
1. K3S目前在国内边缘计算产业发展应用的成熟度如何？
1. K3S是如何保证高并发的？
1. k3s落地案例有哪些
1. k3s的性能如何，可以承载多大业务量
1. kubeedge中cloud core可以对应多少个edgecore
1. kubeedge中edge core可以对应多少个edged
1. 边缘计算的计算几点放在5G基站合适还是下沉UPF合适？
1. 边缘计算落地典型案例有哪些？
1. kubeEdge的网络组件是否要与K8S保持一致？
1. kubevirt是否完成模拟了网络环境？
1. 国内MEC平台解决方案与KubeEdge有何区别？
1. KubeEdge支持的设备接入规模是多大？
1. K3S的轻量级体现在哪里？
1. KubeEdge和K3S哪个在边缘云场景使用的更多？
1. 中心云纳管边缘云和边缘云纳管中心云，这两者各有什么优劣点？
1. K3S与K8S相比，具体轻量化了哪些组建和功能？
1. k3s和k8s集群生命周期管理有什么推荐的方式？
1. KubeEdge端容器和K8S端容器互通有哪些方案推荐
1. k8s crd operator除helm外还有哪些替代产品？
1. 从k3s和k8s的资源环境需求来看，两者的差别不是很大，这种情况下，k3s的差异性优势如何体现？
1. k3s对比k8s安全性能如何？
1. 国内k3s的应用案例有哪些？
1. 虽然边缘计算毫秒级延迟，但边缘计算最吸引客户的是不是设备和数据的独享性和安全性，请问老师如何看待边缘计算的市场优势
1. kubevirt在哪种应用场景下性价比最高
1. K3S常用端口有哪些？
1. k3s怎么避免master的单点故障
1. K3S单节点架构如何保证高可靠？
1. K3S云到边的数据同步对网络有什么要求，是强一致性同步吗
1. MEC中的云适配器，主流架构和软件是什么？
1. OpenYurt除了零侵入外，还有没有其它优点？
1. dvr为啥在生产中使用较少
1. kubevirt在生产是否有大规模部署？
1. upgw和运营商upf有什么区别
1. 云边缘的编排是不是都在云端
1. 虚拟机的镜像是怎么做到增量同步的？
1. etcd本来就容易连接超时，如果考虑安全，用https会更慢，那怎么保证安全？
1. K3S的具体适用场景有哪些？
1. OpenYurt的适用场景有哪些？和K8S,K3S有什么不同点？
1. k3s有好多推荐的书吗
1. 云边缘的pod调度由边缘控制还是云中心控制
1. 边缘云和中心云的主机硬件规格有什么不同
1. EdgeXFoundry有哪些应用场景
1. EdgeX Foundry如何应用在混合云及MEC场景中？
1. Kubeedge与k3s各适用于那些MEC场景？
1. EdgeX Foundry架构中core data和meta data都存储的什么数据？
1. EdgeX Foundry的安全层是怎么做的？
1. 边缘计算有哪些解决方案已经使用。
1. 如何实现redis的高可用
1. 镜像同步到边缘设备是否会造成网络压力
1. k3s在国外有应用场景吗
1. EdgeX Foundry架构中对承载网络的需求是什么?
1. EdgeX Foundry中得数据安全是怎么做的？
1. 三层的分布式云操作系统在实际项目部署中是否存在因为层级导致整体性能下降的情况？
1. openYurt 与 kubeadge 最关键的区别是什么？ 从架构图看不是很明显都是基于两个连接组件将K8s联系起来了
1. kubeedge是否支持arm架构的低功耗产品
1. edgex foundry架构是否支持低功耗产品
1. 运营商边缘环境与IT的边缘环境有什么区别，运营商架构有哪些可以借鉴优化？
1. 已经采用horizon作为界面的系统如何解决日志排查链比较长的问题？
1. 如何通过Core Services Layers和Supporting Services Layers进行安全的访问接入和控制？
1. IP摄像机等设备的视频流如何接入EdgeX Foundry？
1. EdgeXFoundry 必须支持棕色和绿色设备/传感器的现场部署，棕色和绿色分别指什么设备？
1. 目前最有价值的边缘云落地场景有哪些？
1. kubeedge与k3s是否有融合使用方案
1. kubeedge与物联网有没有成熟方案
1. 边缘计算的应用场景有哪些？
1. 边缘计算目前做得好的公司有哪些？
1. Kubesphere集群纳管的数量是否有限制？
1. Kubeedge资源缺乏报错的解决方案是啥？
1. 技术迭代非常快，容器化技术后，是否有其他更新技术的雏形产生？
1. 目前边缘计算有什么应用场景
1. 怎么选择redis的HA实现方式？
1. 边缘设备自动发现有哪些技术？
1. EdgeX Foundry在哪些方面优化时延问题？
1. C比golang性能好，原因是啥？
1. 通常而言，在一个边缘计算应用的场景种，都需要哪些参与方，各方的职责及提供的内容是什么？
1. 在工程实践种，边缘计算哪些方面标准化不足，最亟需标准化的是哪些方面？
1. EdgeXFoundry与kubeEdge面向物联网需求场景有何差异性？
1. 电信运营商边缘云发展有何建议？
1. JWT令牌的优势是什么？
1. k3s/k8s对arm的支持如何？
1. EdgeXFoundry如何保证安全性
1. 随着AIops工具和流程的成熟，云计算行业对运维人员的需求点在哪里
1. k3s有什么好用的工具书或者网站吗？
1. kubedge在物联网卡方面的应用有什么？
1. 一般什么规模下需要考虑边缘计算
1. 边缘计算的架构是否还需要继续发展
1. 服务网格Service Mesh有哪几种实现方式？
1. EdgeXFoundry的四层架构和智慧场馆/开源物业系统案例中的几个层次是如何对应的
