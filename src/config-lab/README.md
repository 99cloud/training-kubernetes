...
1. 准备好 NFS server 机器（另开一台 CentOS
   7.9，单独配一块数据盘），机器规格按需指定。最好挂载一块单独的磁盘。
...
1. 部署步骤
   ...
   1. 如果有额外挂单独的数据盘给 NFS 用，需要格式化这块磁盘并挂载，比如
      vdb。如果没有额外挂盘请忽略这一步
...
如果登陆不了，可以看下是不是 K8S
版本太高了。<https://kubesphere.com.cn/forum/d/2217-account-is-not-active> 安装特定版本的
K8S，参考：<https://stackoverflow.com/questions/49721708/how-to-install-specific-version-of-kubernetes>
